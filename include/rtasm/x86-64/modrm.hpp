...
namespace rtasm {
	namespace x86_64 {
		struct modrm {
			...
			template<typename reg_t> uint8_t rm(disp8 <reg_t> p) { return (static_cast<uint8_t>(p.reg) & 0x7); }
			template<typename reg_t> uint8_t rm(disp32<reg_t> p) { return (static_cast<uint8_t>(p.reg) & 0x7); }
			template<typename reg_t> uint8_t rm(reg_t p) 		 { return (static_cast<uint8_t>(p)     & 0x7); }

			template<typename reg_t> uint8_t mod(disp8 <reg_t> p) { return 0x1; }
			template<typename reg_t> uint8_t mod(disp32<reg_t> p) { return 0x2; }
			template<typename reg_t> uint8_t mod(reg_t p)         { return 0x3; }
